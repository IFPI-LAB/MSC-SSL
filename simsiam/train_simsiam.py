...
parser.add_argument('--sl_pretrain', action='store_true',
                    help='use surpervised pretrain')
parser.add_argument('--num_classes', default=3, type=int, metavar='N',
                    help='number of classes')
parser.add_argument('--save_path', default=r'.\output\bottom_256bs_200epoch', type=str,
                    help='url used to set up distributed training')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if os.path.exists(os.path.join(args.save_path, "log.txt")):
        os.remove(os.path.join(args.save_path, "log.txt"))
    log = open(os.path.join(args.save_path, "log.txt"), mode="a", encoding="utf-8")
    log.writelines('epoch\ttrain_loss\ttrain_acc\ttest_loss\ttest_acc\n')
    log.flush()
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, val_loss = validate(val_loader, model, criterion, args, log)

        # evaluate on train set
        train_acc, train_loss = validate(train_loader, model, criterion, args, log)

        log.writelines('{}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\n'.format(epoch, train_loss, train_acc, val_loss, acc1))
        log.flush()
    ...
    log.writelines('best acc {:.3f} in epoch {}'.format(best_acc1, best_epoch))
    log.flush()
    log.close()
...
def validate(val_loader, model, criterion, args, log):
    ...
    with torch.no_grad():
        ...
        # print(' * Acc@1 {top1.avg:.3f} Acc@3 {top3.avg:.3f}'
        #       .format(top1=top1, top3=top3), file=log)
