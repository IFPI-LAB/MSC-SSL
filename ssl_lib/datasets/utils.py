...
def get_tobacco(root, cfg):
    ...
    if cfg.labeled_folder is not None:
        labeled_root=os.path.join(root, 'labeled', cfg.labeled_folder)
        labeled_data = LoadedImageFolder(labeled_root)
        labeled_data = {"images": labeled_data.data, "labels": labeled_data.targets}
